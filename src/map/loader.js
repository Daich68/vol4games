...
let ready=false,returning=false;
...
catch{document.querySelector('.load-note').textContent='Без регистрации. Сохранение прогресса недоступно в этом браузере.';}
document.getElementById('loadProgress').textContent=count?`Прочитано ${count} из 4 · можно продолжить с карты`:'Начни с любого произведения';
function open(){
 if(!ready)return;
...
window.addEventListener('vol4:map-ready',()=>{
 ready=true;clearTimeout(slow);pre.dataset.state='ready';status.textContent='Карта готова';
 enter.disabled=false;enter.textContent=count?'Продолжить чтение':'Открыть карту';retry.hidden=true;
 if(returning)open();
},{once:true});
