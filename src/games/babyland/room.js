...
export function buildRoom(scene){
 ...
 const chrome=material('#c6c9dc',.7),wood=material('#c4abc7'),pink=material('#ad5486'),lightPink=material('#e4c1d8');
 ...
 const tile=document.createElement('canvas');tile.width=128;tile.height=128;const ctx=tile.getContext('2d');ctx.fillStyle='#e5dfea';ctx.fillRect(0,0,128,128);ctx.fillStyle='#b7a9c0';ctx.fillRect(0,0,64,64);ctx.fillRect(64,64,64,64);const map=new T.CanvasTexture(tile);map.colorSpace=T.SRGBColorSpace;map.wrapS=map.wrapT=T.RepeatWrapping;map.repeat.set(12,12);map.magFilter=T.NearestFilter;
 const floor=add(new T.PlaneGeometry(24,24),new T.MeshStandardMaterial({map,roughness:.55}),[0,-.085,0]);floor.rotation.x=-Math.PI/2;
 box(material('#d2c2d9'),[0,2.25,-2.7],[14,4.6,.15]);
 ...
 const rug=add(new T.CylinderGeometry(1.1,1.1,.015,64),material('#c38daf'),[0,-.068,0]);rug.scale.z=.8;
 return group;
}
