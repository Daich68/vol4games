...
## Исправления чтения и скримера — 2026-09-08

Актуализирует предыдущие проверки: стартовый экран удалён, примерочная запускается сразу. Звук активируется первым действием пользователя.

Проверено в Chromium (SwiftShader), 1280×900 и 390×844:
- Скример виден без модального окна; окно реакции открывается после его завершения. Проверены DOM-порядок показа и снимок экрана.
- Стих выводится целиком с исходными переносами, остаётся после 120 секунд, доступен повторно через архив. На мобильной ширине шрифт 18 px, есть вертикальная прокрутка, горизонтального переполнения нет.
- При надетой wrong-вещи 120 секунд чтения не увеличивают наказание. После закрытия текста следующий шаг давления происходит через полный интервал 9 секунд.
- Ошибок JavaScript в проверенном сценарии нет.
- `npm.cmd run build` успешно; остаётся предупреждение Vite о размере общего чанка Three.js.
- `PYTHONUTF8=1 python tools/check_brief.py`: семь текстов совпадают с PDF, расхождений статической проверки нет.

Сценарии проверки и снимки: `C:/Users/daich/Documents/vol4games-backups/check-reader-scream.mjs`, `check-reading-pressure.mjs`, `scream-uncovered.png`, `reader-mobile.png`. Эти проверки не заменяют тестирование на реальном мобильном GPU и проверку звука на слух.

### Полноценная сцена скримера

Скример длится 2,4 секунды: темнота, удар на 336 мс, крупная гримаса, удержание и затухание. Один визуальный удар без циклических вспышек. Отдельная фронтальная камера снимает текущее лицо с надетыми предметами независимо от ракурса примерочной. На время сцены управление блокируется; затем открывается окно реакции. При reduced motion остаётся только окно. Звук синтезируется WebAudio: провал музыки, низкий удар, диссонанс и затухающий полосовой шум; использует общий регулятор выключения звука.

Проверки: `check-scream-full.mjs` — скример не перекрыт, чтение и мобильная вёрстка без регрессии, ошибок JS нет. Визуально просмотрен кадр `scream-full.png`. Звук на слух в этом проходе не оценивался.
